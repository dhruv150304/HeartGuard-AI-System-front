    ...
    <div className="flex min-h-screen bg-[#FDF8F3]">
      ...
      <main className="flex flex-1 items-center justify-center px-6 py-10">
        <form onSubmit={submit} className="w-full max-w-md rounded-2xl bg-white p-7 shadow-sm sm:p-10">
          ...
          <p className="mt-2 text-sm text-stone-500">{mode === "signIn" ? "Sign in securely to your health dashboard." : "Patient accounts require email verification."}</p>
          ...
          <button disabled={loading} className="mt-7 w-full rounded-xl bg-[#982016] px-5 py-3.5 text-sm font-bold text-white disabled:opacity-60">{loading ? "Please wait…" : mode === "signIn" ? "Sign in" : "Create patient account"}</button>
          ...
          <p className="mt-7 text-center text-sm text-stone-500">{mode === "signIn" ? "New patient?" : "Already have an account?"} <button type="button" onClick={() => { setMode(mode === "signIn" ? "signUp" : "signIn"); setError(""); setMessage(""); }} className="font-bold text-red-800">{mode === "signIn" ? "Create an account" : "Sign in"}</button></p>
